<header>
	<nav class="navbar">
		<div class="container">
			<div class="navbar-header">
				<button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#navbar" aria-expanded="false">
	            <span class="sr-only">导航折叠</span>
	            <span class="fa fa-bars text-nav"></span>
	          </button>
				<a href="/" class="brand">
					<img src="/images/logo.png">
				</a>
			</div>
			<div class="navbar-collapse collapse" id="navbar">
				<ul class="nav navbar-nav navbar-right">
					<li>
						<a href="/">主页</a>
					</li>
					<li class="dropdown">
                <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">产品 <span class="caret"></span></a>
                <ul class="dropdown-menu">
                  <li class="dropdown-header">健康微生态</li>
									<li><a href="/microbiome" class="text-l-b-s">简介</a></li>
									<li><a href="/baba">粑粑说</a>	</li>
									<li><a href="/baby">Infant说</a></li>
									<li role="separator" class="divider"></li>
									<li class="dropdown-header">临床微生态</li>
									<li><a href="/pathogen" class="text-l-b-t">简介</a></li>
                  <li><a href="/pathogen#product">血元谱</a></li>
                  <li><a href="/pathogen#product">脑元谱</a></li>
									<li role="separator" class="divider"></li>
                  <li class="dropdown-header">营养微生态</li>
                </ul>
              </li>
					<li>
						<a href="/us">我们</a>
					</li>
					<li>
						<a href="/cooperation">合作</a>
					</li>
					<li>
						<a href="/job">招聘</a>
					</li>

				</ul>
			</div>
		</div>
	</nav>
</header>
